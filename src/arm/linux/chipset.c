 ...
 *
 * @param[in] ro_soc_model - ro.soc.model value.
 *
 * @returns Decoded chipset name. If chipset could not be decoded, the resulting
 * structure would use `unknown` vendor and series identifiers.
 */
struct cpuinfo_arm_chipset cpuinfo_arm_android_decode_chipset_from_ro_soc_model(
	...
	if (ro_soc_model[0] != '\0') {
		if (strncmp(ro_soc_model, "Tensor", 6) == 0) {
			...
			return chipset;
		} else if (strncmp(ro_soc_model, "GS201", 5) == 0) {
			chipset.vendor = cpuinfo_arm_chipset_vendor_google;
			chipset.series = cpuinfo_arm_chipset_series_google_tensor;
			strncpy(chipset.suffix, "G2", 2);
			return chipset;
		} else if (strncmp(ro_soc_model, "SM", 2) == 0) {
			chipset.vendor = cpuinfo_arm_chipset_vendor_qualcomm;
			chipset.series = cpuinfo_arm_chipset_series_qualcomm_snapdragon;
			strncpy(chipset.suffix, ro_soc_model, CPUINFO_ARM_CHIPSET_SUFFIX_MAX);
			return chipset;
		}
	}
